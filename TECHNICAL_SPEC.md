...
2. Таймер Pomodoro
   ...
   - Поддержка ambient‑музыки: пользователь может загрузить mp3 в ресурс `res/raw` и выбрать трек в настройках. Музыка проигрывается в цикле во время работы таймера и автоматически отключается при паузе/остановке.
   - Настраиваемый финиш‑тон: пользователь может выбрать встроенный mp3‑файл или системный звук для сигнала по окончании таймера.
...
- Компонент воспроизведения звука: MediaPlayer или ExoPlayer для ambient и финальных тонов; логика — в отдельном SoundManager/AudioService (опционально при использовании foreground service).
...
- Ambient‑музыка начинается вместе с таймером и останавливается при паузе/остановке; возможен выбор трека в настройках.
- Звуковой сигнал по окончании проигрывается корректно и не конфликтует с фоновой музыкой.
